...
- 编译器
  负责将模板字符串编译成js渲染函数。
- 运行时
  负责创建Vue.js实例，渲染视图和使用虚拟DOM实现重新渲染，基本包含除了编译器外的所有部分。
- UMD
  UMD版本可以通过<script>标签直接在浏览器中使用。
- CommonJS
  CommonJS版本用来配合比较旧的打包工具，如browserify或webpack1。
- ES Module
  ES Module版本用来配合现代打包工具，比如webpack2或rollup。

### Vue.js的整体目录结构

  vue.js整体分为三个部分：核心代码（原型方法与API方法）、跨平台相关（web或weex不同平台下的渲染API）、公用工具函数。

  在vue的分层架构中：最底层为普通的Vue构造函数，构造函数上一层为添加在prototype 的一些原型方法，再往上即添加在构造函数的全局API方法。这里再往上层即涉及到跨平台相关内容：选择平台（web或weex），将特定于不同平台的代码加载到构建文件中；往上一层即渲染层——包含服务器端渲染相关的内容和编译器相关内容（此层内容可选）。最顶层为入口，也可以叫做出口：对构建工具和vue.js的使用者来说，这是入口；对于vue.js自身来说，这是出口。

  [vue.js的程序结构](./imgs/vue-program-structure.png)


## vue 的实例方法与全局API

  Vue 的实例方法是放在Vue.prototype上的，例如vm.$nextTick（Vue.prototype.$nextTick）
  ，而全局API则是添加到Vue构造函数上的，例如Vue.nextTick

### 与数据相关的实例方法

  与数据相关的实例方法有3个：vm.$watch、wm.$set、wm.$delete

### 与事件相关的实例方法

  与事件相关的实例方法有4个：vm.$on、vm.$off、vm.$once、vm.$emit
...
#### vm.$on的实现原理
  ...
  vm.$on用于监听当前实例上的自定义时间，事件可以由vm.$emit触发。回调函数会接收所有传入事件所触发的函数的额外参数。

  事件的实现方式为：在注册事件时将回调函数手机起来，在触发事件时将收集起来的回调函数依次调用即可。注意：在实例上会存在一个_events对象，用来存储事件，它在Vue进行初始化操作时就会被创建在实例上。
...
#### vm.$off的实现原理
  ...
  vm.$off用于移除自定义事件监听器，如果没有提供参数，则移除所有的事件监听器；若只提供了事件，则移除该事件所有监听器；若同时提供了事件与回调，则只移除该回调的监听器。
...
#### vm.$once的实现原理
  ...
  vm.$once用于监听一个自定义事件，但只触发一次，在第一次触发之后移除监听器。
  vm.$once与vm.$on的区别在于vm.$once只能被触发一次。
  实现思路为：在vm.$once中调用vm.$on实现监听自定义事件功能，当自定义事件触发后会执行拦截器，将监听器从事件列表中移除。
...
#### vm.$emit的实现原理
  ...
  vm.$emit用于触发当前实例上的事件。附加参数都会传给监听器回调。
  实现思路：使用事件名从vm._events中取出对应的事件监听器回调函数列表，然后依次执行列表中的监听器回调并将参数传递给监听器回调。

### 与生命周期相关的实例方法

  与生命周期相关的实例方法有4个：vm.$mount、vm.$forceUpdate、vm.$nextTick、vm.$destroy

#### vm.$forceUpdate

  vm.$forceUpdate作用为迫使vue.js实例重新渲染，注意它仅仅影响实例本身以及插入插槽内容的子组件，而非所有子组件。

  vue.js的每一个实例都有一个watcher，只需执行实例watcher的update方法，即可让实例重新渲染。

#### vm.$destroy

  vm.$destroy作用为完全销毁一个实例，它会清理该实例与其他实例的连接，并解绑其全部指令及监听器，同时会触发beforeDestroy和destroyed的钩子函数。

  销毁实例的逻辑：首先清理当前组件与父组件之间的连接——即将当前组件实例从父组件实例的￥children属性中删除即可。
  【注意】一个组件可以同时被多个组件引入，但就算是同一类组件，在不同的父组件引入后，属于是不同的vue.js实例，所以只需从父级的子组件列表中销毁当前这个子组件实例即可。
