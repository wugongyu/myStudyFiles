...
#### 请求方法
  ...
  在web应用中，常见的请求方法是GET和POST，除此之外，还有HEAD、DELETE、PUT、CONNECT等方法。**请求方法存在于报文的第一行的第一个单词，通常是大写**。

  HTTP_Parser在解析请求报文的时候，将报文头抽取出来，设置为req.method。

#### 路径解析
  
  路径部分存在于报文的第一行的第二部分。Http_Parser将其解析为req.url。客户端代理（浏览器）会将该地址解析成报文，将路径和查询部分放在报文第一行。【注意】hash部分会被丢弃，不会存在于报文的任何地方。

#### 查询字符串

  查询字符串位于路径之后，这部分内容经常需要为业务逻辑所用，node提供了querystring模块来处理这部分数据。
  【注意】若查询字符串中的键出现多次，那么它的值会是一个数组。
  ```js
  // foo=bar&foo=baz
...
#### Cookie
  ...
  【注意】HTTP是一个无状态的协议，而**现实中的业务却是需要一定的状态的，否则无法区分用户之间的身份**。cookie则是最早用于**标识和认证一个用户**的方案。Cookie能够记录服务器于客户端之间的状态。
...
客户端发送的Cookie在请求报文的Cookie字段中，HTTP_Parser会将所有的报文字段解析到req.headers上，那么Cookie就是req.headers.cookie。

  服务端可通过更改Set-Cookie字段的值，是的客户端在收到带Set-Cookie的响应后，在之后的请求时在Cookie字段中带上这个值。

  Set-Cookie字段的格式
  ```
  Set-Cookie: name=value;Path=/;Expires=Sun,26-Aug-26 09:01:35 GMT; Domain=.domain.com;
  ```
  其中，name=value为必须包含部分，其余部分为可选参数：

- path
  表示该Cookie影响到的路径，当前访问路径不满足该匹配时，浏览器不发送该cookie。
- Expires与Max-Age
  用来告知浏览器Cookie的过期时间。若**不设置该选项，关闭浏览器时会丢失该Cookie**。
  若设置了过期时间，浏览器会**将Cookie内容写入磁盘中并保存**，下次打开浏览器依旧有效。
  Expires的值为UTC格式的时间字符串，表示的是过期时间点（服务器端与客户端时间不匹配时，这种**具体时间点的设置就会存在偏差**）；Max-Age则告知浏览器Cookie在多久后过期（即相对时间）
- HttpOnly
  该参数告知浏览器不允许通过脚本document.cookie来更改该Cookie值。
  注意，设置了HttpOnly之后，该值在document.cookie中不可见，但在HTTP请求中，依然会发送这个Cookie到服务器端。
- Secure
  Secure为true时，表示创建的Cookie只能在HTTPS连接中被浏览器传递到服务器端进行会话校验，如果是HTTP连接则不会传递该信息，所以很难被窃听到。

##### Cookie的性能影响

  由于Cookie的实现机制，一旦服务器端向客户端发送了设置Cookie的意图，除非Cookie过期，否则客户端每次请求都会发送这些Cookie到服务器端，**一旦设置的Cookie过多，将会导致报头较大，大多数的Cookie并不需要每次都用上，因此这会造成带宽的浪费**。

  性能优化规则：

  1. 减小Cookie的大小
  【注意】如果域名的根节点设置Cookie，几乎所有子路径下的请求都会带上这些Cookie，而对于静态文件而言，器业务定位几乎不关心状态，Cookie对它而言几乎是无用的。
  2. 为静态组件使用不同的域名
  为不需要Cookie的组件更换域名可以减少无效Cookie的传输。更换域名使得业务相关的Cookie不再影响静态文件，还可以突破浏览器下载线程数量的限制。但是【注意】将域名转换为IP需要进行DNS查询，多一个域名就多一次DNS查询。
  3. 减少DNS查询
  减少DNS查询与使用不同的域名似乎是两条冲突规则，但现今浏览器都会进行DNS缓存，由此可削弱使用不同域名而产生的副作用的影响。

##### Cookie的应用

  广告和在线统计领域最为依赖Cookie，通过嵌入第三方的广告或统计脚本，将Cookie与当前页面绑定，一次表示用户，得到用户的浏览行为，因此广告商即可定向投放广告。

#### Session

  【注意】Cookie除了体积过大这一显著的问题，还有一个最为严重的问题，即**前后端都可以修改Cookie，因此数据就极容易被篡改和伪造**。所以Cookie对于敏感数据的保护是无效的。

  为解决Cookie敏感数据问题，Session应运而生。
  **Session只保留在服务器端**，客户端无法修改，这样数据安全性得到保障，数据也无需在协议中每次被传递。

  将每个客户与服务器中的session数据一一对应起来的方式：
  
  1. 基于Cookie实现用户和数据的映射
  将口令放在cookie中，即使cookie会被篡改而丢失了映射关系，但也无法修改服务器端存在的数据，且session有效期很短，若在有效期内客户端和服务器端没有交互产生，服务器端就将数据删除。
  一旦服务器端启用session，将约定一个值（随意约定，但需为唯一且不重复的值）作为session的口令，每个请求到来时，检查Cookie中的口令与服务器端的数据，如果过期，就重新生成，当然重新生成session还不足以完成整个流程，还需要在响应给客户端时设置新的值，以便下次请求时能够对应服务器端的数据。

  2. 通过查询字符串来实现浏览器端和服务器端数据的对应
  原理为检查请求的查询字符串，如果没有值，会先生成新的带值的URL，然后形成跳转，让客户重新发起请求。这样，新的请求到来时就能通过session的检查，除非内存中的数据过期。
  【注意】**这种方案带来的风险远大于基于Cookie实现的风险**，因为只要将地址栏中的地址发给另一个人，那么他就拥有跟你相同的身份。
  3. 利用HTTP请求头中的ETag来处理session
  更换浏览器和电脑后无效。
