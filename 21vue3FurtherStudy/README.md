...
在进⾏服务端渲染时，组件的初始化流程与客户端渲染时组件的
初始化流程基本⼀致，但有两个重要的区别。

- 服务端渲染的是应⽤的当前快照，它不存在数据变更后重新渲染的情况。因此，**所有数据在服务端都⽆须是响应式的**。利⽤这⼀点，我们可以减少服务端渲染过程中创建响应式数据对象的开销。

- 服务端渲染只需要获取组件要渲染的 subTree 即可，⽆须调⽤渲染器完成真实 DOM 的创建。因此，在服务端渲染时，**可以忽略“设置 render effect 完成渲染”这⼀步**。

### 客户端激活的原理

由于浏览器在渲染了由服务端发送过来的 HTML 字符串之后，⻚⾯中已经存在对应的 DOM 元素了，所以组件代码在客户端运⾏时，不需要再次创建相应的 DOM 元素。但是，组件代码在客户端运⾏时，仍然需要做两件重要的事：

- 在⻚⾯中的 DOM 元素与虚拟节点对象之间建⽴联系；
  一个虚拟节点被挂载之后，为了保证更新程序能正确运行，需通过该虚拟节点的vnode.el属性存储对真实DOM对象的引用。

- 为⻚⾯中的 DOM 元素添加事件绑定。
  在服务端渲染的过程中，会忽略虚拟节点中与事件相关的props，故在客户端运行时，需要将这些事件正确地绑定到元素上。
